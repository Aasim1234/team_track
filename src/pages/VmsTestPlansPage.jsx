    ...
    <div className="min-h-screen bg-gray-900 text-white flex">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="p-6 space-y-4">
          ...
          <BentoCard className="p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[13px] font-semibold text-white">Test Scenarios</p>
              {canAuthor && (
                <button onClick={() => setShowAddItem(true)} className="flex items-center gap-1.5 text-[12px] text-blue-500 hover:text-blue-400 font-medium">
            ...
            <div className="space-y-2">
              {items.map((item) => (
                <div key={item.id} className="flex items-start gap-3 px-3 py-2 border border-gray-600 rounded-md">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-[13px] text-white font-medium">{item.title}</span>
                      {item.category && <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-700 text-gray-400 flex-shrink-0">{item.category}</span>}
                    </div>
                    {item.notes && <p className="text-[12px] text-gray-500 mt-0.5">{item.notes}</p>}
                  </div>
                  {canAuthor ? (
                    <select
                      value={item.status}
                      onChange={(e) => updateItemStatus(item.id, e.target.value)}
                      className="text-[11px] bg-transparent border border-gray-600 rounded-md px-1.5 py-1 outline-none flex-shrink-0"
                    >
                      {Object.entries(TEST_PLAN_ITEM_STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                    </select>
                  ) : (
                    <StatusBadge domain={TEST_PLAN_ITEM_STATUS} value={item.status} size="sm" />
                  )}
                  {canDelete && (
                    <button onClick={() => deleteItem(item.id)} className="text-gray-500 hover:text-red-500 flex-shrink-0">
                      <Trash2 size={13} />
                    </button>
                  )}
                </div>
              ...
              {items.length === 0 && (
                <p className="text-[12px] text-gray-500">
                  No scenarios yet — add your VMS test scenarios here, or share your test plan document and they'll be added for you.
                </p>
              )}
            </div>
          </BentoCard>
